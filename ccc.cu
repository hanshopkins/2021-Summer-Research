...
float* cross_correlations(const float* vecA_re, const float* vecA_im, const float* vecB_re, const float* vecB_im, const unsigned int len, const int startShift, const int stopShift) //len is the length of the input vectors
{
	if (stopShift >= startShift)
	{
		...
		*/
		...
		dim3 dimBlock; dimBlock.x = 16; dimBlock.y = 16; dimBlock.z = 2;
		dim3 dimGrid; dimGrid.x = (len + 15) / 16; dimGrid.y = (len + 15) / 16; dimGrid.z = 1;
		
		CCKernel << <dimGrid, dimBlock >> > (d_vecA_re, d_vecA_im, d_vecB_re, d_vecB_im, len, d_multValues_re, d_multValues_im, startShift, N);

		//we don't need the input vectors on the device any more
		cudaFree(d_vecA_re);
		cudaFree(d_vecA_im);
		cudaFree(d_vecB_re);
		cudaFree(d_vecB_im);
		...
		float* outputVec = (float*)malloc(outputSize);
	...
	else
	{
		...
		float* outputVec = (float*)malloc((stopShift - startShift + 1) * sizeof(float));
		return outputVec;
	}
}